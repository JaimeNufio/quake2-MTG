...
qboolean Pickup_Weapon(edict_t *ent, edict_t *other)
{
	...
	if (!(ent->spawnflags & DROPPED_ITEM))
	{
		...
		//JAIME NUFIO
		//No ammo on given/pickup weapon
...
void NoAmmoWeaponChange(edict_t *ent){
	...
	if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Red"))]
	...
	if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Black"))] > 1
		&& ent->client->pers.inventory[ITEM_INDEX(FindItem("super shotgun"))])
	{
	//	ent->client->newweapon = FindItem("super shotgun");
		return;
	}
	if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Black"))]
	...
	if (ent->client->weaponstate == WEAPON_READY)
	{
		if (((ent->client->latched_buttons | ent->client->buttons) & BUTTON_ATTACK))
		{
			...
			{ 
				...
				//
				//gi.cprintf(ent, PRINT_HIGH, "Can't cast this spell, you lack %s mana.\n", ent->client->pers.weapon->ammo);
				//timer :eyes:
